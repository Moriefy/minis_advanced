package com.minis.haprial.provider.thinking
...
object ThinkingRuleResolver {
    ...
    fun anthropicThinkingShape(
        ...
        val adaptive = com.minis.haprial.provider.anthropic.AnthropicProvider
            .modelUsesAdaptiveThinking(modelId)
        if (level.isEnabled && supportsReasoning != false) {
            if (adaptive) {
                return mapOf(
                    "effort" to com.minis.haprial.provider.anthropic.AnthropicProvider
                        .thinkingEffort(level),
                )
            }
            val budget = com.minis.haprial.provider.anthropic.AnthropicProvider

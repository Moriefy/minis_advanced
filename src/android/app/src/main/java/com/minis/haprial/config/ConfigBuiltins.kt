package com.minis.haprial.config

import android.content.Context
import com.minis.haprial.config.collections.EnvVarsCollection
import com.minis.haprial.service.DynamicIslandSupport
import com.minis.haprial.config.collections.GroupsCollection
import com.minis.haprial.config.collections.ModelsCollection
import com.minis.haprial.config.collections.ProvidersCollection
import com.minis.haprial.config.fields.ClosureField
import com.minis.haprial.config.fields.PrefsBoolField
import com.minis.haprial.config.fields.PrefsDoubleField
import com.minis.haprial.config.fields.PrefsEnumField
import com.minis.haprial.config.fields.PrefsIntCodedEnumField
import com.minis.haprial.config.fields.PrefsIntField
import com.minis.haprial.config.fields.PrefsLongField
import com.minis.haprial.config.fields.PrefsStringField
import com.minis.haprial.config.fields.ReadOnlyField
...
import com.minis.haprial.ui.chat.ChatViewModelStore
...
internal object ConfigBuiltins {
    ...
    private fun registerAppearance(r: ConfigRegistry, context: Context) {
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = prefs,
                key = com.minis.haprial.ui.settings.KEY_SHOW_CHAT_TITLE,
                defaultValue = true,
            )
        )
        r.register(
    ...
    private fun registerChat(r: ConfigRegistry, context: Context) {
        ...
        val appearancePrefs = context.getSharedPreferences(
            com.minis.haprial.ui.settings.PREF_APPEARANCE,
        ...
        r.register(
            PrefsBoolField(
                ...
                key = com.minis.haprial.ui.settings.KEY_TOOL_PREVIEW,
                defaultValue = true,
            )
        )
        ...
        r.register(
            PrefsBoolField(
                ...
                prefs = appearancePrefs,
                key = com.minis.haprial.ui.settings.KEY_AUTO_FOCUS_AFTER_REPLY,
                defaultValue = true,
            )
        )
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = com.minis.haprial.ui.settings.KEY_FONT_CHAT_INPUT,
        ))
        r.register(fontScaleField(
            ...
            prefs = appearancePrefs,
            key = com.minis.haprial.ui.settings.KEY_FONT_MESSAGE,
        ))
    }
    ...
    private fun registerLogs(r: ConfigRegistry, context: Context) {
        ...
        r.register(
            ClosureField(
                ...
                revertable = true,
                reader = {
                    ConfigValue.Bool(com.minis.haprial.logging.AppLogger.isEnabled(context))
                },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    com.minis.haprial.logging.AppLogger.setEnabled(context, b)
                },
            )
        )
    }
    ...
    private fun registerProviderCollections(
        ...
        r.register(com.minis.haprial.config.collections.ThinkingRulesCollection(providerRepo))
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val providersById = HashMap<String, com.minis.haprial.data.model.ProviderInstance>(cfg.instances.size)
        ...
        r.register(
            ClosureField(
                ...
                risk = ConfigRisk.SENSITIVE,
                revertable = true,
                reader = { ConfigValue.Bool(com.minis.haprial.data.EnvVarPrivacyStore.isEnabled) },
                writer = { v ->
                    val b = (v as? ConfigValue.Bool)?.value
                        ?: throw ConfigError.TypeMismatch("bool")
                    com.minis.haprial.data.EnvVarPrivacyStore.setEnabled(b)
                },
            )
        )
        ...
        r.register(
            ReadOnlyField(
                ...
                reader = {
                    val cfg = providerRepo.config.value
                    val entriesById =
                        HashMap<String, com.minis.haprial.data.model.ModelEntry>(cfg.modelEntries.size)
                    for (e in cfg.modelEntries) entriesById[e.id] = e
                    val providersById =
                        HashMap<String, com.minis.haprial.data.model.ProviderInstance>(cfg.instances.size)
                    for (inst in cfg.instances) providersById[inst.id] = inst
                    ConfigValue.Arr(
    ...
    private fun registerSoul(r: ConfigRegistry, context: Context) {
        ...
        // English ≤ 500 words); see [com.minis.haprial.agent.SoulStore.isOverLimit].
        ...
        fun loadCurrent(): com.minis.haprial.agent.SoulFile =
            com.minis.haprial.agent.SoulStore.load(context)
                ?: com.minis.haprial.agent.SoulMDParser.parse(
                    com.minis.haprial.agent.SoulStore.DEFAULT_CONTENT
                )

        fun saveCurrent(file: com.minis.haprial.agent.SoulFile) {
            com.minis.haprial.agent.SoulStore.save(context, file)
        }
            ...
            ClosureField(
                ...
                writer = { v ->
                    ...
                    val name = raw.trim().ifBlank {
                        com.minis.haprial.agent.SoulMetadata.DEFAULT.name
                    }
                    val cur = loadCurrent()
            ...
            ClosureField(
                ...
                reader = {
                    ...
                    ConfigValue.Str(cur.metadata.lang.ifBlank {
                        com.minis.haprial.agent.SoulMetadata.DEFAULT.lang
                    })
                },
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
        ...
        r.register(
            ClosureField(
                ...
                description = "Personality / voice instructions injected as a block in the system prompt. Length cap is language-aware: Chinese ≤ ${com.minis.haprial.agent.SoulStore.CHINESE_CHAR_LIMIT} chars OR English ≤ ${com.minis.haprial.agent.SoulStore.ENGLISH_WORD_LIMIT} words (rule picked by the majority language). The writer also rejects prompt-injection patterns (\"ignore previous instructions\" etc.) — keep this to genuine character / tone guidance.",
                ...
                writer = { v ->
                    val raw = (v as? ConfigValue.Str)?.value
                        ?: throw ConfigError.TypeMismatch("string")
                    if (com.minis.haprial.agent.SystemPromptBuilder
                    ...
                    val check = com.minis.haprial.agent.SoulStore.isOverLimit(raw)
                    when (check) {
                        is com.minis.haprial.agent.SoulBodyLimitCheck.Ok -> Unit
                        is com.minis.haprial.agent.SoulBodyLimitCheck.OverLimitChinese ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.chars} Chinese chars exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} chars, or rewrite mostly in English to use the ≤ ${com.minis.haprial.agent.SoulStore.ENGLISH_WORD_LIMIT}-word cap."
                            )
                        is com.minis.haprial.agent.SoulBodyLimitCheck.OverLimitEnglish ->
                            throw ConfigError.InvalidValue(
                                "Over limit: ${check.words} English words exceeds cap of ${check.cap}. Either trim to ≤ ${check.cap} words, or rewrite mostly in Chinese to use the ≤ ${com.minis.haprial.agent.SoulStore.CHINESE_CHAR_LIMIT}-char cap."
                            )
                    }
                    val cur = loadCurrent()

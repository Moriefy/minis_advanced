...
import com.minis.haprial.MinisApp
...
internal object ProviderDebugMethods {
            ...
            if (includeMembers) {
                ...
                for (memberId in group.memberEntryIds) {
                    ...
                    val entry = cfg.modelEntries.find { it.id == memberId }
                        ?: com.minis.haprial.data.model.SystemVoiceEntries.resolve(memberId)
                        ?: continue
    ...
    suspend fun quickTest(context: Context, params: JSONObject): JSONObject {
        ...
        val entry = repo.config.value.modelEntries.find { it.id == entryId }
            ?: com.minis.haprial.data.model.SystemVoiceEntries.resolve(entryId)
            ?: throw RPCException(-32602, "Model entry not found: $entryId")

        val requested = params.optJSONArray("kinds")
        val kinds: List<com.minis.haprial.ui.components.QuickTestKind> = if (requested == null) {
            com.minis.haprial.ui.components.applicableKinds(entry)
        } else {
            (0 until requested.length()).mapNotNull { i ->
                when (requested.optString(i).lowercase()) {
                    "text" -> com.minis.haprial.ui.components.QuickTestKind.TEXT
                    "speechout", "speech_out", "tts" -> com.minis.haprial.ui.components.QuickTestKind.SPEECH_OUT
                    "transcription", "asr" -> com.minis.haprial.ui.components.QuickTestKind.TRANSCRIPTION
                    "imagegen", "image_gen", "image" -> com.minis.haprial.ui.components.QuickTestKind.IMAGE_GEN
                    else -> null
                }
            }
        }
        ...
        for (kind in kinds) {
            ...
            val state = try {
                com.minis.haprial.ui.components.performTest(kind, entry, repo, context)
            } catch (e: Exception) {
                com.minis.haprial.ui.components.QuickTestState.Failure(
            ...
            results.put(JSONObject().apply {
                ...
                when (state) {
                    is com.minis.haprial.ui.components.QuickTestState.TextReply -> {
                        put("status", "ok"); put("detail", state.text.take(400))
                    }
                    is com.minis.haprial.ui.components.QuickTestState.AudioReply -> {
                        put("status", "ok")
                        put("bytes", state.data.size)
                        put("detail", "audio ${state.data.size} bytes")
                    }
                    is com.minis.haprial.ui.components.QuickTestState.ImageReply -> {
                        put("status", "ok")
                        put("bytes", state.data.size)
                        put("detail", "image ${state.data.size} bytes")
                    }
                    is com.minis.haprial.ui.components.QuickTestState.Failure -> {

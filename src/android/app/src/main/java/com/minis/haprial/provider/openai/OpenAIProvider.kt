package com.minis.haprial.provider.openai
...
import com.minis.haprial.data.model.LLMMediaAttachment
import com.minis.haprial.data.model.LLMMessage
import com.minis.haprial.data.model.LLMModel
import com.minis.haprial.data.model.LLMResponse
...
import com.minis.haprial.provider.thinking.ThinkingResolveContext
import com.minis.haprial.provider.thinking.ThinkingRuleResolver
import com.minis.haprial.provider.LLMProvider
import com.minis.haprial.provider.applyUserAgentOverride
...
import com.minis.haprial.provider.failOnSilentEmptyCompletion
...
class OpenAIProvider private constructor(
     ...
     * user-authored custom rules from [com.minis.haprial.provider.thinking.ThinkingRuleResolver]'s
    ...
    private val client = OkHttpClient.Builder()
        ...
        .connectionPool(com.minis.haprial.network.NetworkMonitor.sharedLLMConnectionPool)
    ...
    private fun rawStreamMessage(
        ...
        val memBefore = com.minis.haprial.diagnostics.MemorySnapshot.capture()
        val serStartNs = System.nanoTime()
        val bodyStr = try {
            body.toString()
        } catch (t: Throwable) {
            com.minis.haprial.diagnostics.LargeAllocProbe.report(
                "openai.body.toString", -1, "model=${model.id} messages=${messages.size}",
                memBefore, serStartNs, failure = t,
            )
            throw t
        }
        if (bodyStr.length >= com.minis.haprial.diagnostics.LargeAllocProbe.NOTABLE_BYTES) {
            com.minis.haprial.diagnostics.LargeAllocProbe.report(
        ...
        if (!response.isSuccessful) {
            ...
            com.minis.haprial.logging.AppLogger.error(
                "OpenAIProvider",
            ...
            if (com.minis.haprial.BuildConfig.DEBUG) {
                com.minis.haprial.debug.LLMRequestLog.add(
                    com.minis.haprial.debug.LLMRequestLog.Entry(
                        provider = "openai",
                ...
                run {
                    ...
                    if (delta != null) {
                        ...
                        if (cLen + rcLen + rLen + tcLen > 0 || delta.has("role")) {
                            com.minis.haprial.logging.AppLogger.debug(
                                "OpenAIProvider",
                                    ...
                                    when (part) {
                                        ...
                                        is AgentContentPart.ImageData -> {
                                            if (supportsImages) {
                                                ...
                                                val safeBytes = com.minis.haprial.provider.ImageBudget.compressUnderBudget(part.data)
    ...
    private fun responsesImageBlock(
        ...
        // T-imgsize: provider-boundary backstop.
        val safeBytes = com.minis.haprial.provider.ImageBudget.compressUnderBudget(data)
    ...
    private fun buildResponsesAPIBody(
        ...
        if (com.minis.haprial.data.FastModePrefs.isEnabled() &&
                        ...
                        if (imgParts.isNotEmpty()) {
                            ...
                            for (part in msg.contentParts) {
                                when (part) {
                                    ...
                                    else -> com.minis.haprial.logging.AppLogger.error(
                                        "OpenAIProvider",

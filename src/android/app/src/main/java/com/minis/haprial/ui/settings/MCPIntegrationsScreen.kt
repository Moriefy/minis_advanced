...
import com.minis.haprial.ui.components.DialogTextField
...
fun MCPIntegrationsScreen(
    ...
    envVarRepository: com.minis.haprial.data.repository.EnvVarRepository? = null,
) {
...
private fun MCPAddSheet(
    mcpRepository: MCPRepository,
    envVarRepository: com.minis.haprial.data.repository.EnvVarRepository?,
...
private fun MCPFormTab(
    ...
    val appEnvEntries by (envVarRepository?.entries
        ?.collectAsState() ?: remember { mutableStateOf(emptyList<com.minis.haprial.data.repository.EnvVarRepository.EnvVarEntry>()) })
    ...
    var oauthClientSecret by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.minis.haprial.mcp.oauth.MCPOAuthStore.clientSecret(context, it)
            } ?: "",
        )
    }
    ...
    var oauthAuthorized by remember {
        mutableStateOf(
            editServer?.id?.let {
                com.minis.haprial.mcp.oauth.MCPOAuthStore.isAuthorized(context, it)
            } ?: false,
        )
    }
    var oauthBusy by remember { mutableStateOf(false) }

    fun currentOAuthConfig(): com.minis.haprial.mcp.oauth.MCPOAuthConfig? {
        val cfg = com.minis.haprial.mcp.oauth.MCPOAuthConfig(
                    ...
                    if (oauthAuthorized) {
                        MinisTextButton(
                            ...
                            onClick = {
                                val sid = name.trim()
                                if (sid.isNotEmpty()) {
                                    com.minis.haprial.mcp.oauth.MCPOAuthStore.signOut(context, sid)
                        ...
                        onClick = {
                            ...
                            when {
                                ...
                                cfg == null ->
                                    ...
                                    com.minis.haprial.mcp.oauth.MCPOAuthStore.setClientSecret(
                                        context, sid, oauthClientSecret.trim().ifBlank { null },
                                    )
                                    oauthBusy = true
                                    oauthScope.launch {
                                        val result = com.minis.haprial.mcp.oauth.MCPOAuthController(context)
                                            .authorize(sid, url.trim(), cfg)
                                        oauthBusy = false
                                        when (result) {
                                            is com.minis.haprial.mcp.oauth.MCPOAuthController.Result.Success -> {
                                            ...
                                            is com.minis.haprial.mcp.oauth.MCPOAuthController.Result.Cancelled -> {}
                                            is com.minis.haprial.mcp.oauth.MCPOAuthController.Result.Failed -> {

package com.minis.haprial.sandbox.offload
...
import com.minis.haprial.provider.safeOptString
import com.minis.haprial.sandbox.NativeOffloadHandler
import com.minis.haprial.sandbox.NativeOffloadRequest
import com.minis.haprial.sandbox.NativeOffloadResult
...
class ModelUseOffloadHandler(
    ...
    private fun cmdRun(args: OffloadArgs, request: NativeOffloadRequest): NativeOffloadResult {
        ...
        if (hasAudioInput && provider !is com.minis.haprial.provider.openai.OpenAIProvider) {
            return NativeOffloadResult(
                2,
        ...
        if (ptSpec.active) {
            val openAI = provider as? com.minis.haprial.provider.openai.OpenAIProvider
                ?: return NativeOffloadResult(
                    2,
        ...
        (provider as? com.minis.haprial.provider.openai.OpenAIProvider)?.let { openAI ->
    ...
    private data class ImageGenConfig(
        ...
        val endpointOverride: com.minis.haprial.data.model.ImageEndpointMode? = null,
    )
    ...
    private fun parseImageGenConfig(inputJson: String): ImageGenConfig {
        ...
        val endpoint = when (rawEndpoint.lowercase()) {
            "auto" -> com.minis.haprial.data.model.ImageEndpointMode.auto
    ...
    private fun performRawPassthrough(
        ...
        entry: ModelEntry,
        openAI: com.minis.haprial.provider.openai.OpenAIProvider,
        systemPrompt: String?,
    ...
    private fun tryImageGenerationRoute(
        ...
        provider: com.minis.haprial.provider.LLMProvider,
        inputJson: String,
        ...
        if (instance.credentialType == com.minis.haprial.data.model.ProviderCredential.oauth) return null
        ...
        val effectiveMode = when {
            ...
            isPureImageGenerator -> com.minis.haprial.data.model.ImageEndpointMode.imagesGenerations
            instance.imageEndpointMode == com.minis.haprial.data.model.ImageEndpointMode.auto &&
        ...
        if (effectiveMode == com.minis.haprial.data.model.ImageEndpointMode.chatCompletions) return null
        ...
        if (effectiveMode == com.minis.haprial.data.model.ImageEndpointMode.imagesGenerations) {
            val response = try {
                runBlocking {
        ...
        val response = try {
            runBlocking {
                ...
                providerRepository.setImageEndpointResolved(
                    instance.id, com.minis.haprial.data.model.ImageEndpointMode.imagesGenerations,
                )
            }
        } catch (e: Throwable) {
            val msg = e.message ?: ""
            val routeMissing = (e is com.minis.haprial.data.model.LLMError.ProviderError ||
                e is com.minis.haprial.data.model.LLMError.InvalidApiKey) && looksLikeEndpointMissing(msg)
            if (routeMissing) {
                Log.i(TAG, "[ModelUseRoute] auto probe → route missing (${msg.take(120)}) — caching chat_completions, falling back")
                providerRepository.setImageEndpointResolved(
                    instance.id, com.minis.haprial.data.model.ImageEndpointMode.chatCompletions,
                )
    ...
    private fun writeImageResult(
        entry: ModelEntry,
        response: com.minis.haprial.data.model.LLMResponse,
        outputPath: String?,
        outputExt: String,
        endpointUsed: String,
        sessionId: String?,
    ...
    private fun entryDict(entry: ModelEntry): JSONObject =
        com.minis.haprial.offload.ModelUseManager.entryDict(
            entry,

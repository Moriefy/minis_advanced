...
import com.minis.haprial.auth.OpenAIOAuthManager
import com.minis.haprial.auth.OpenRouterOAuthManager
...
import com.minis.haprial.ui.components.RowLabel
...
fun AddProviderScreen(
    ...
    var selectedVoiceTemplate by remember {
        mutableStateOf<com.minis.haprial.data.model.VoiceProviderTemplate?>(null)
    }
...
private fun ChooseProviderScreen(
    ...
    onSelectVoiceTemplate: (com.minis.haprial.data.model.VoiceProviderTemplate) -> Unit = {},
) {
    SettingsScaffold(
        ...
        val templates = com.minis.haprial.data.model.VoiceProviderTemplate.all
        ...
        SettingsSection(
            ...
            templates.forEachIndexed { index, template ->
                val capabilityRes = when (template.capability) {
                    com.minis.haprial.data.model.VoiceProviderTemplate.Capability.TTS ->
                        R.string.add_provider_voice_capability_tts
                    com.minis.haprial.data.model.VoiceProviderTemplate.Capability.ASR ->
                        R.string.add_provider_voice_capability_asr
                    com.minis.haprial.data.model.VoiceProviderTemplate.Capability.BOTH ->
...
private fun ConfigureProviderScreen(
    ...
    providerRepository: ProviderRepository,
    voiceTemplate: com.minis.haprial.data.model.VoiceProviderTemplate? = null,
    onBack: () -> Unit,
    onSaved: () -> Unit,
) {
                            ...
                            try {
                                when (providerType) {
                                    ProviderType.kimiCode -> {
                                        ...
                                        val key = com.minis.haprial.auth.KimiOAuthManager.login(
                                    ...
                                    ProviderType.anthropic -> {
                                        val key = com.minis.haprial.auth.ClaudeOAuthManager.login(context, pendingInstanceId, providerRepository)
                                        maskedToken = maskOAuthToken(key)
                                    }
                                    ProviderType.openAI -> {
                                    ...
                                    ProviderType.xAI -> {
                                        val key = com.minis.haprial.auth.XAIOAuthManager.login(context, pendingInstanceId, providerRepository)
                                        maskedToken = maskOAuthToken(key)
                                    }
                                    else -> {
                                ...
                                // signal.
                                errorMessage = if (e is com.minis.haprial.auth.OAuthNetworkUnreachableException) {

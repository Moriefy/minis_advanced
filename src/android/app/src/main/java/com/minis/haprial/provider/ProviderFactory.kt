package com.minis.haprial.provider
...
import com.minis.haprial.provider.anthropic.AnthropicProvider
import com.minis.haprial.provider.gemini.GeminiProvider
import com.minis.haprial.provider.openai.OpenAIProvider
        ...
        val provider: LLMProvider = when (instance.providerType) {
            ...
            ProviderType.openAI -> {
                ...
                val manualBearer = if (context != null &&
                    ...
                    com.minis.haprial.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    ...
                    OpenAIProvider(
                        oauthTokenProvider = { oauthManager.validAccessToken() ?: throw com.minis.haprial.data.model.LLMError.InvalidApiKey() },
                        model = model,
            ...
            ProviderType.xAI -> {
                ...
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.minis.haprial.auth.XAIOAuthManager(context, instance.id)
                    OpenAIProvider.oauthOpenAICompat(
                        oauthTokenProvider = {
                            oauthManager.validAccessToken()
                                ?: throw com.minis.haprial.data.model.LLMError.InvalidApiKey()
                        },
                        model = model,
                        basePath = base,
                    )
            ...
            ProviderType.kimiCode -> {
                ...
                val base = basePath ?: "${com.minis.haprial.auth.KimiDeviceFlow.CODING_API_BASE}/v1"
                val manualBearer = if (context != null &&
                    instance.credentialType == ProviderCredential.oauth) {
                    com.minis.haprial.auth.OAuthManager.forInstance(context, instance)?.loadManualBearerToken()
                } else null
                if (instance.credentialType == ProviderCredential.oauth && manualBearer.isNullOrEmpty()
                    && context != null) {
                    val oauthManager = com.minis.haprial.auth.KimiOAuthManager(context, instance.id)

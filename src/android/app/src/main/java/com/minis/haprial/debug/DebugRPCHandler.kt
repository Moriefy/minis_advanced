package com.minis.haprial.debug
...
class DebugRPCHandler(private val context: Context) {
    ...
    private suspend fun handleShellExecute(params: JSONObject): JSONObject {
        ...
        val result = try {
            ExecutionCoordinator.execute(
                ...
                lineCallback = { rawLine ->
                    val (_, urls) = com.minis.haprial.terminal.MinisUrlMarker.extract(rawLine)
        ...
        for (raw in capturedUrls) {
            com.minis.haprial.terminal.MinisOpenUrlBroker.offer(raw)
        }
        return JSONObject()
    ...
    private suspend fun handleUpdateCheck(): JSONObject {
        return when (val r = com.minis.haprial.data.UpdateChecker.check()) {
            is com.minis.haprial.data.UpdateChecker.CheckResult.UpdateAvailable -> JSONObject()
            ...
            com.minis.haprial.data.UpdateChecker.CheckResult.UpToDate ->
                JSONObject().put("status", "up_to_date")
            com.minis.haprial.data.UpdateChecker.CheckResult.NoReleaseAvailable ->
                JSONObject().put("status", "no_release")
            is com.minis.haprial.data.UpdateChecker.CheckResult.NoApkAsset ->
                JSONObject().put("status", "no_apk_asset").put("tag_name", r.tagName)
            com.minis.haprial.data.UpdateChecker.CheckResult.Forbidden ->
                JSONObject().put("status", "forbidden")
            com.minis.haprial.data.UpdateChecker.CheckResult.NetworkUnreachable ->
                JSONObject().put("status", "network_unreachable")
            is com.minis.haprial.data.UpdateChecker.CheckResult.Error ->
    ...
    private suspend fun handleUpdateDownload(params: JSONObject): JSONObject {
        ...
        return when (val r = com.minis.haprial.data.UpdateChecker.download(context, url)) {
            is com.minis.haprial.data.UpdateChecker.DownloadResult.Success -> JSONObject()
                .put("status", "ok")
                .put("path", r.file.absolutePath)
                .put("size", r.file.length())
            is com.minis.haprial.data.UpdateChecker.DownloadResult.Error -> JSONObject()
                .put("status", "error")
    ...
    private fun handleUpdateInstall(params: JSONObject): JSONObject {
        ...
        val canInstall = com.minis.haprial.data.UpdateChecker.canInstall(context)
        return try {
            com.minis.haprial.data.UpdateChecker.installApk(context, file)
    ...
    private fun handleWriteFile(params: JSONObject): JSONObject {
        ...
        val hostFile = PRootKernel.resolveHostPath(path) ?: run {
            val rootfsDir = com.minis.haprial.sandbox.RootfsManager.getInstance(context).rootfsDir
    ...
    private fun handlePermissionsList(): JSONObject {
        ...
        for (tool in com.minis.haprial.offload.OffloadPermissionManager.toolRegistry) {
            arr.put(JSONObject()
                ...
                .put("currentLevel", com.minis.haprial.offload.OffloadPermissionManager.getLevel(tool.toolName).name)
    ...
    /**
     * T344: Direct invocation of [com.minis.haprial.sandbox.offload.ShizukuOffloadHandler]
    ...
    private fun handleShizukuExec(params: JSONObject): JSONObject {
        ...
        val handler = com.minis.haprial.sandbox.offload.ShizukuOffloadHandler(context)
        ...
        val request = com.minis.haprial.sandbox.NativeOffloadRequest(
            pid = -1,
    ...
    /**
     * Direct invocation of [com.minis.haprial.sandbox.offload.ModelUseOffloadHandler]
    ...
    private fun handleModelUseExec(params: JSONObject): JSONObject {
        ...
        val finalArgv: List<String> = if (params.has("input")) {
            ...
            val hostFile = com.minis.haprial.sandbox.PRootKernel.resolveHostPath(linuxPath)
        ...
        val app = context.applicationContext as com.minis.haprial.MinisApp
        val handler = com.minis.haprial.sandbox.offload.ModelUseOffloadHandler(context, app.providerRepository)
        val request = com.minis.haprial.sandbox.NativeOffloadRequest(
            pid = -1,
     ...
     * [com.minis.haprial.sandbox.offload.SessionsOffloadHandler] for e2e
    ...
    private fun handleSessionsExec(params: JSONObject): JSONObject {
        ...
        val app = context.applicationContext as com.minis.haprial.MinisApp
        val handler = com.minis.haprial.sandbox.offload.SessionsOffloadHandler(app.chatRepository)
        val request = com.minis.haprial.sandbox.NativeOffloadRequest(
            pid = -1,
        ...
        return when (sub) {
            "set" -> {
                ...
                kotlinx.coroutines.runBlocking {
                    com.minis.haprial.config.ConfigBridge.performWriteBatch(
                        items = items,
            ...
            "get" -> {
                ...
                com.minis.haprial.config.ConfigBridge.readField(
                    path = path,
                    filter = filter,
                    page = page,
                    pageSize = pageSize,
            ...
            "topics" -> JSONObject().apply {
                put("ok", true)
                put("topics", com.minis.haprial.config.ConfigBridge.allTopics())
            }
            "topic-help" -> {
                ...
                val known = com.minis.haprial.config.ConfigBridge.allTopics()
                    .let { arr -> (0 until arr.length()).any { arr.optString(it) == topic } }
                if (!known) {
                    throw RPCException(-32602, "Unknown topic '$topic' — call subcommand=topics")
                }
                val fields = com.minis.haprial.config.ConfigBridge.fieldsForTopic(topic)
                JSONObject().apply {
                    put("ok", true)
                    put("topic", topic)
            ...
            "audit-list" -> {
                ...
                com.minis.haprial.config.ConfigBridge.auditList(limit, scope)
            }
